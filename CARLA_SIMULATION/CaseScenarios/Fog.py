...
def main():
    ...
    # Create WeatherParameters object for foggy weather
    foggy_weather = carla.WeatherParameters(
        cloudiness=60.0,               # Partially cloudy sky
        precipitation=0.0,            # No rain
        precipitation_deposits=0.0,   # No water on the ground
        sun_altitude_angle=20.0,      # Low sunlight
        fog_density=80.0,             # High fog density
        fog_distance=10.0,            # Very low visibility
        wetness=0.0,                  # Dry roads
        wind_intensity=5.0            # Slight breeze
    )
    
    # Apply the foggy weather
    set_weather(client, foggy_weather)
